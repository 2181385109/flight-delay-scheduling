...
- **Delay reduction 0.48 min/high-risk-flight vs a 1.0 min target** (≈ 4% off
  the day's total *predicted* delay). Real predicted delay is driven mostly by
  network state, which a ±30-minute slot shift barely changes — so the
  achievable gain from re-timing alone is genuinely small. Note this is a
  reduction in *predicted* delay, not measured real-world delay. (An earlier
  synthetic version, where congestion was an artificially dominant cause,
