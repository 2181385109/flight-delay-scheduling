...
## ⚠️ Read this before trusting those numbers

**The table above is measured on synthetic data generated by this repo**, not on
real flights. The labels come from a formula in `data/synth.py`, and its noise
level was tuned until the capture target was met — so the model is largely
recovering a known equation. Those numbers say the pipeline works; they do **not**
say delays are predictable to 3.7 minutes.

So the same pipeline was also run on **real data** — nycflights13 (real 2013
EWR/JFK/LGA departures from US DOT/BTS + real hourly weather), 26,483 flights,
3,141 real aircraft, no code change beyond the loader:

| Metric | Synthetic | **Real data** | |
|---|---|---|---|
| Prediction MAE | 3.74 min | **20.85 min** | 5.6× worse; only 9.6% better than predicting the mean (23.05) |
| PR-AUC | 0.774 | **0.458** | |
| High-risk capture (Recall) | 0.829 | **0.736** | **misses the ≥0.80 target** (still 6× the rule baseline's 0.122) |
| Constraint satisfaction | 0.998 | **1.000** | ✅ transfers unchanged — deterministic feasibility |
| High-risk delay reduction | 3.04 min | **1.02 min** | reduction in *predicted* delay, not measured delay |

On real data even RandomForest (MAE 26.59) loses to the mean baseline — the
learnable signal is genuinely weak, because the dominant real causes (ATC flow
control, crew/connection, upstream network state, mechanical) are absent from
the dataset. Full analysis and caveats:
**[`outputs/reports/real_data_benchmark.md`](outputs/reports/real_data_benchmark.md)**
(reproduce with `python scripts/benchmark_real_data.py`).

**What actually generalizes** is the engineering — leakage-safe features,
out-of-fold evaluation, the CP-SAT constraint model, tests and CI — all of which
ran on real data unchanged.
